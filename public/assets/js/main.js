...
function windowLoadInit() {
	...
	jQuery('.embed-placeholder').each(function(){
		jQuery(this).on('click', function(e) {
			e.preventDefault();
			jQuery(this).replaceWith('<iframe class="embed-responsive-item" src="'+ jQuery(this).attr('href') + '?rel=0&enablejsapi=1&autoplay=1'+ '"></iframe>');
		});
	});
...
function toggleVideo(state) {  
    ...
    jQuery('.popupVid').find('iframe').each(function(){
	    ...
	    jQuery(this)[0].contentWindow.postMessage('{"event":"command","func":"' + func + '","args":""}', '*');
    })
}